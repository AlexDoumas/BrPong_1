...
parameters = {'asDORA': True, 'gamma': 0.3, 'delta': 0.1, 'eta': 0.9, 'HebbBias': 0.5,'bias_retrieval_analogs': True, 'use_relative_act': True, 'run_order': ['cdr', 'selectTokens', 'r', 'wp', 'm', 'p', 's', 'f', 'c'], 'run_cyles': 5000, 'write_on_iteration': 100, 'firingOrderRule': 'random', 'strategic_mapping': False, 'ignore_object_semantics': False, 'ignore_memory_semantics': True, 'mag_decimal_precision': 0, 'exemplar_memory': False, 'recent_analog_bias': True, 'lateral_input_level': 1, 'screen_width': 1200, 'screen_height': 700, 'doGUI': False, 'testing': True, 'GUI_update_rate': 50, 'starting_iteration': 0, 'tokenize': False, 'ho_sem_act_flow': 0, 'remove_uncompressed': False, 'remove_compressed': False}
...
class RunMenu(object):
                    ...
                    if run_command.upper() == 'V':
                        ...
                        while go_on_cdr:
                            ...
                            if ch_dr_state.upper() == 'V':
                                ...
                                while go_on_driver:
                                    print ('')
                                    top_unit = input('Add a (G)roup, (P), (RB), (PO) to driver, or (E)xit>')
                                    if top_unit.upper() == 'G':
                                ...
                                while go_on_recipient:
                                    print ('')
                                    top_unit = input('Add a (G)roup, (P), (RB), (PO) to recipient, or (E)xit>')
                                    if top_unit.upper() == 'G':
                                        # enter a group and all it's child tokens.
                                        # pass for now.
                                        pass
    ...
    def terminal_view(self, memory):
        ...
        while go_on:
            print ('')
            type_view = input('View network (A)ctivations, (N)etwork state, (M)appings, or (B)ack to Run_Menu>')
