...
def add_objects(memory, objects): 
    ...
    for obj in objects:
        width = round(random.random()*100)
        hight = round(random.random()*100)
        objs_sym[0]['RBs'].append({'pred_name': 'non_exist', 'pred_sem': [], 'higher_order': False, 'object_name': obj, 'object_sem': [['x_ext', 1, 'x_ext', 'nil', 'state'], ['x_ext'+str(width), 1, 'x_ext', width, 'value'], ['y_ext', 1, 'y_ext', 'nil', 'state'], ['y_ext'+str(hight), 1, 'y_ext', hight, 'value'], ['total_ext', 1, 'total_ext', 'nil', 'state'], ['total_ext'+str(width*hight), 1, 'total_ext', width*hight, 'value']], 'P': 'non_exist'})
    # make the new analog. 
...
# function to run a simple NB trial. 
def run_NBtrial(network, start, tally, correct):
    # make some objects. 
    obj_analog, network.memory = add_objects(network.memory, ['tree1', 'tree2'])
    # select a dimension.
    ...
    if len(prop) > 0:
        if prop[0].my_type == 'P':
            ...
            for myRB in prop[0].myRBs:
                my_semantics = [link.mySemantic.name for link in myRB.myPred[0].mySemantics]
            ...
            tally['RB'] += 1
            my_semantics = [link.mySemantic.name for link in prop[0].myPred[0].mySemantics]
            if dimension[0] in my_semantics and dimension[1] in my_semantics:
                correct = 1
            print('RB', correct)
        else:
    ...
    obj_analog, network.memory = apply_preds(obj_analog, objs, prop, dimension[0], network.memory)
    # sample a prop from LTM to enter recipient; call it sampled_analog. 
    sampled_rep = sample_LTM_rep(network.memory, dimension, start, threshold=0.95)
    sampled_analog = sampled_rep[0].myanalog
    # put obj_analog in the driver and sampled_analog in the recipient. 
    network.memory = basicRunDORA.add_tokens_to_set(network.memory, network.memory.analogs.index(obj_analog), 'analog', 'driver')
    network.memory = basicRunDORA.add_tokens_to_set(network.memory, network.memory.analogs.index(sampled_analog), 'analog', 'recipient')
...
def run_NBsim(trials, memory_states):
    ...
    for memory_state in memory_states:
        ...
        for i in range(trials):
            tally, was_correct = run_NBtrial(network, start_POs, tally, correct)
            correct += was_correct
        print(tally, '\n', correct, '\n\n\n')
