...
class LargeAgent:
  ...
  def getAction(self, gameState):
    #add a frame to the frame counter
    self.currentFrame += 1
  ...
  def update(self,oldState,action,newState,reward,terminal):
    ...
    if(self.states.shape[0] == self.updateSize):
      ...
      loss,_ = self.sess.run([self.loss,self.dummy],feed_dict={self.gameState:self.states,self.disiredOutput:self.actionLabels})
