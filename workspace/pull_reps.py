...
def sample_item_from_state(state_array, criterion):
    ...
    while go_on:
        ...
        for currentRB in prop[0]['RBs']:
            # if you have an RB or relation, then look for the critical semantic among the pred_semantics. 
            if len(currentRB['pred_sem']) > 0:
                for semantic in currentRB['pred_sem']:
                    if (semantic[0] == criterion) and (semantic[1]>.9):
                        go_on = False
                ...
                # otherwise, look for the critical semantic among the obj_semantics. 
                for semantic in currentRB['object_sem']:
                    if (semantic[0] == criterion) and (semantic[1]>.9):
                        go_on = False
                        break
...
for state_array in mem_state_list:
    # for each criterion in criteria, sample 10 items from the state array. 
    sampled_this_state = []
    for criterion in criteria:
        sampled_reps = []
        for iteration in range(10):
            # with probability .8, sample from the 100 most recently learned items, otherwise, sample from the entire array of items (i.e., all of crit_array[4]). 
            rand_num = random.random()
            if rand_num <= .9:
                start_point = len(state_array)-100
            ...
            sampled_reps.append(sampled_item[0]) # NOTE: You are appending sample[0] because random.sample() returns a list, so the sample is a one item list containing the sampled dict. 
