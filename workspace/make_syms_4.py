    ...
    for kind in range(prop_type['kinds']):
        for element in range(prop_type['number']):
            ...
            if prop_type['prototype_variety'] == 'so':
                ...
                counter = 0
                # select 1 same and 3 different. 
                dim_picker = round(random.random()*3)
                if dim_picker == 0:
                    samedim = 'height'
                elif dim_picker == 1:
                    samedim = 'width'
                ...
                for RB in new_sym_dict['RBs']:
                    ...
                    for dimension in dimensions:
                        if dimension == samedim:
                            vital_sem.append([dimension, 1, dimension, 'nil', 'state'])
                            vital_sem.append([dimension+str(same_val), 1, dimension, same_val, 'value'])
                        else:
                            vital_sem.append([dimension, 1, dimension, 'nil', 'state'])
                            dim_val = round(random.random()*9)+1
                            vital_sem.append([dimension+str(dim_val), 1, dimension, dim_val, 'value'])
