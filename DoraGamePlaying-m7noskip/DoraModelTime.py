...
class DoraAgent:
    ...
    def setupTables(self):
        ...
        for dim in self.possibleDimensions:
            ...
            for pred in self.possiblePredicates:
                for traj in self.possiblePreds2:
                    for rel in self.possiblePreds3:
                        action = pred + traj + rel
    ...
    def selectAction(self,dimension,predicate, traj, traj_rel):
        action_name = predicate + traj + traj_rel
    ...
    def getAction(self, gameState):
        ...
        if self.lastGameStates is not None:
            action = self.selectAction(self.lastChosenDim,self.lastChosenPred, self.lastTrajPaddle, self.lastTrajRel)
        else:
            action = np.random.randint(0,3)
    ...
    def update(self,oldState,action,newState,reward,terminal):
        ...
        if lastTrajRel is not None and self.lastTrajRel is not None:
            action_name = newPred + lastTrajPaddle + lastTrajRel
            ...
            action_name2 = self.lastChosenPred + self.lastTrajPaddle + self.lastTrajRel 
