...
class SmallAgent:
  ...
  """
  def __init__(self,inputSize=8, trajectory=False):
    self.actionsTaken = 0
    ...
    self.inputSize = inputSize + 8 * self.trajectory
    #build the tensor graph
    self.buildNetwork(self.inputSize)
  ...
  def encodeTrajectory(self,vector):
    addition = []
    velocityIndx = []
    
    if(len(vector) == 12):
      velocityIndx = [4,5,10,11]
    else:
      velocityIndx = [2,3,6,7]
      
    for i in velocityIndx:
      if(vector[i] > 0):
	addition.extend([1,0])
      elif(vector[i] < 0):
	addition.extend([0,1])
    ...
    vector = np.concatenate((vector,addition),axis=0)
    return vector
  ...
  def getAction(self, gameState):
    ...
    if(self.currentFrame%self.skipFrame == 0):
      ...
      if(self.trajectory):
	gameState = self.encodeTrajectory(gameState)
      neuralInput = np.expand_dims(gameState,axis=0)
      if(neuralInput.shape[1] == self.inputSize):
	...
	print "input size does not match",neuralInput.shape
	self.lastAction = 0
  ...
  def update(self,oldState,action,newState,reward,terminal):
    ...
    if(self.states is None):
      ...
      if(self.trajectory):
	newState = self.encodeTrajectory(newState)
