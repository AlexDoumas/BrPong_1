...
def dim_comparator(val1, val2, name1, name2, dim_string, tolerance=2):
    """
    ...
    Args:
        ...
        tolerance: level of tolerance for the comparator (int).
            According to my experiments 2is the ideal level of tolerance.
...
def relational_states_breakout(f_vec1, f_vec2=None, cross_compatible=False, cross_dict=False):
    """
    ...
    if cross_compatible:
        # Ball, time 1
        ball1_x = -(210 - f_vec1[3])  # invert OpenCv y-coordinate
        ball1_y = -f_vec1[2]

        # Paddle, time 1
        paddle1_x = -(210 - f_vec1[5])  # invert OpenCv y-coordinate
        paddle1_y = -f_vec1[4]
    ...
    if f_vec2 is not None:
        ...
        if cross_compatible:
            # Ball, time 2
            ball2_x = -(210 - f_vec2[3])  # invert OpenCv y-coordinate
            ball2_y = -f_vec2[2]

            # Paddle, time 2
            paddle2_x = -(210 - f_vec2[5])  # invert OpenCv y-coordinate
            paddle2_y = -f_vec2[4]

        # Calculate trajectory for ball only if there is a ball in the first screen
        if ball_present:
            if cross_dict:
                traj_ball = dim_comparator(ball2_y, ball1_y, 'b2', 'b1', 'trajy')
            else:
                traj_ball = dim_comparator(ball2_x, ball1_x, 'b2', 'b1', 'trajx')
            relations.append(traj_ball)

        # Object relations (only available if there is a ball)
        if ball_present:
            if cross_dict:
                dim = dim_comparator(ball2_y, paddle2_y, 'b', 'p', 'y')
            else:
                dim = dim_comparator(ball2_x, paddle2_x, 'b', 'p', 'x')
            relations.append(dim)
...
def relational_states_pong(f_vec1, f_vec2=None, cross_compatible=False, cross_dict=False):
    """
    Builds a relational description of the pong screen. When there is only
    a single screen available (1st time step) the description doesn't include
    relations across time.
